...
function init() {
  ...
  var g_material11 = new THREE.MeshLambertMaterial({
    map: texture,
    ambient: 0xbbbbbb
  });
  var g_material01 = new THREE.MeshLambertMaterial({
    ambient: 0xbbbbbb,
    vertexColors: THREE.VertexColors
  });
  var g_material00 = new THREE.MeshLambertMaterial({ ambient: 0xbbbbbb });
  //var g_material11 = new THREE.MeshToonMaterial({ wireframe: true });
...
function createHexSphere(iNumSubdiv) {
  ...
  var hexoplanetGeometry = new THREE.IcosahedronGeometry(radius, 0);
  ...
  for (var pataticas = 0; pataticas < iNumSubdiv; pataticas++) {
    ...
    for (var i = 0; i < hexoplanetGeometry.faces.length; i++) {
      ...
      for (var id = 0; id < aCurrentVertexID.length; id++) {
        ...
        var v1 = directionVector.subSelf(
...
function doRefreshGeometry() {
  ...
  g_hexoplanetGeometry.computeCentroids();
